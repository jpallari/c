...
int test_slice_span() {
    ...
    assert_eq(s1.len, 4L, "%ld", "slice 1 length 4");
    assert_eq(s2.len, 4L, "%ld", "slice 2 length 4");
    assert_eq(s1.buffer[0], 12, "%d", "slice 1 index 0");
    assert_eq(s1.buffer[1], 13, "%d", "slice 1 index 1");
    assert_eq(s1.buffer[2], 14, "%d", "slice 1 index 2");
    assert_eq(s1.buffer[3], 15, "%d", "slice 1 index 3");
    assert_eq(s2.buffer[0], 12, "%d", "slice 1 index 0");
    assert_eq(s2.buffer[1], 13, "%d", "slice 1 index 1");
    assert_eq(s2.buffer[2], 14, "%d", "slice 1 index 2");
    assert_eq(s2.buffer[3], 15, "%d", "slice 1 index 3");
...
int test_slice_equal() {
    ...
    assert_true(jp_slice_equal(s1_1, s1_1), "equals with itself");
    assert_false(jp_slice_equal(s1_1, s1_2), "not equals with shifted");
    assert_false(
        jp_slice_equal(s1_1, s1_3), "not equals with different length"
    );
    assert_true(jp_slice_equal(s1_1, s2_1), "equals with similar");
...
int test_slice_from() {
    ...
    assert_eq(s1.len, 13L, "%ld", "s1 len must be size of string");
    assert_eq(
        s2.len, sizeof(int) * 4, "%ld", "s2 len must be size of array"
    );
    assert_eq_cstr(
        (char *)s1.buffer, "hello world!", "s1 contents should be the same"
    );
    assert_eq_bytes(
        s2.buffer, (u8 *)arr, s2.len, "s2 contents should be the same"
    );
...
int test_slice_copy_larger() {
    ...
    assert_eq_bytes(
        s1.buffer, &expected_arr1[2], s1.len, "s1 should change"
    );
    assert_eq_bytes(
        arr2, expected_arr2, sizeof(arr2), "arr2 should not change"
    );
    assert_eq_bytes(
        s2.buffer, &expected_arr2[1], s2.len, "s2 should not change"
    );
...
int test_slice_move_overlapping() {
    ...
    assert_eq_bytes(arr, expected_arr, sizeof(arr), "arr should change");
    assert_eq_bytes(
        s1.buffer, &expected_arr[2], s1.len, "s1 should change"
    );
...
int test_slice_from_cstr_unsafe() {
    ...
    assert_eq(
        slice.len, 13L, "%ld", "slice len must match string length"
    );
    assert_eq(
