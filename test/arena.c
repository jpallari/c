...
void test_arena(test *t) {
    ...
    allocation s_buf1 = alloc_new(&alloc, int, buf1_count);
    int *buf1 = s_buf1.ptr;
    ...
    allocation s_buf2 = alloc_new(&alloc, int, buf2_count);
    int *buf2 = s_buf2.ptr;
    ...
    // try to overallocate
    allocation s_buf3 = alloc_new(&alloc, int, 3);
    assert_false(t, allocation_exists(s_buf3), "overallocation must fail");
}
